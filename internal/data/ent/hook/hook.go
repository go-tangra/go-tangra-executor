...
// The ScriptAssignmentFunc type is an adapter to allow the use of ordinary
// function as ScriptAssignment mutator.
type ScriptAssignmentFunc func(context.Context, *ent.ScriptAssignmentMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ScriptAssignmentFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ScriptAssignmentMutation); ok {
		return f(ctx, mv)
	}
